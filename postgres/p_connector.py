...
def postgres_connector(func):
    ...
    def inner(*args, **kwargs):
        logger.debug(f"CONNECTING TO POSTGRES WITH SSH MODE {SSH_MODE}")
        ...
        if SSH_MODE == 1:
            ...
            server.start()
            logger.debug("POSTGRES SSH TUNNEL STARTED")
        ...
        try:
            ...
            logger.error(f"!!! POSTGRES ERROR -- {ex}")
        finally:
            conn.close()
            logger.debug("POSTGRES DISCONNECTED")
            if SSH_MODE == 1:
                server.stop()
                logger.debug("POSTGRES SSH TUNNEL DISCONNECTED")
...
def postgres_demo_query(connector):
    ...
    curr.execute("SELECT * FROM statistics.market_leader_polygons LIMIT 10")
    data = curr.fetchall()
    curr.close()
    logger.info(data)
    return data
